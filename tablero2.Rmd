...
#devtools::install_github("dgonxalex80/paquetePROB", force = TRUE)
library(paquetePROB)
...
```{r}
ggplot(CarreraLuz22, aes(x = edad, y = timerun)) +
  geom_point(alpha = 0.5, color = "#0073C2FF") +
  theme_minimal() +
  labs(title = "Relación entre Edad y Tiempo de Carrera", x = "Edad", y = "Tiempo (segundos)")
```
...
```{r}
ggplot(CarreraLuz22, aes(x = categoria, y = timerun, fill = categoria)) +
  geom_boxplot(alpha = 0.7) +
  theme_minimal() +
  labs(title = "Tiempo de Carrera por Categoría", x = "Categoría", y = "Tiempo (segundos)") +
...
df_sex <- CarreraLuz22 %>%
  ...
  mutate(percentage = round(n / sum(n) * 100, 1),
         label = paste0(percentage, "%"))  # Formato de etiqueta

# Crear gráfico de pastel con etiquetas de porcentaje
ggplot(df_sex, aes(x = "", y = n, fill = sex)) +
  geom_bar(stat = "identity", width = 1) +  # Crear el pastel
  coord_polar(theta = "y") +  # Convertir a circular
  theme_minimal() +
  labs(title = "Distribución de Género") +
  scale_fill_manual(values = c("#0073C2FF", "#EFC000FF")) +
  geom_text(aes(label = label), position = position_stack(vjust = 0.5), size = 5, color = "white") +
  theme(axis.text.x = element_blank(),  # Ocultar ejes innecesarios
